use crate::{
    BombOf4, Card, CardSuit, CardValue, FullHouse, Pair, Sequence, SequenceBomb, SequenceOfPairs,
    Single, Trio, ValidCardCombo, MAX_CARDS_IN_HAND,
};
use itertools::Itertools;

pub fn get_card_combination(
    prev_combo: Option<&ValidCardCombo>,
    cards: &Vec<Card>,
    user_id_who_played_cards: &str,
) -> Option<ValidCardCombo> {
            ...
            return if let Some(prev_combo) = prev_combo {
                if let ValidCardCombo::Single(prev_single) = prev_combo {
                    match prev_single
                        .cards
                        .first()
                        .expect("Every single card should have a Vec of cards")
                        .suit
                    {
    ...
    if cards.len() == 5 {
        ...
        if includes_phoenix {
            let mut cards_without_phoenix: Vec<Card> = cards
                ...
                .map(|card| (*card).clone())
                .collect();
            let highest_value_card = cards_without_phoenix
                .last()
                .expect("Should have non-Phoenix cards")
                .clone();
    ...
    if cards.len() >= 5 {
        ...
        if includes_phoenix {
            ...
            if let Some(i_of_last_continuos_sequence_card) = i_of_last_continuos_sequence_card {
                ...
                let mut new_card = cards_without_phoenix
                    .last()
                    .expect("There should be non-Phoenix cards")
                    .clone();
                new_card.value = new_card.value.add(1);
...
pub fn next_combo_beats_prev(prev: &Option<&ValidCardCombo>, next: &ValidCardCombo) -> bool {
    if let Some(prev) = prev {
        ...
        if let ValidCardCombo::Single(Single {
            cards: prev_cards,
            value: prev_value,
            ..
        }) = &prev
...
pub fn get_user_can_play_wished_for_card(
    prev_combo: Option<&ValidCardCombo>,
    users_hand: &Vec<Card>,
    wished_for_card_value: &CardValue,
) -> bool {
    ...
    let mut all_combos_of_same_length: Vec<Vec<Card>> = users_hand
        .iter()
        .combinations(prev_combo.cards().len())
        .map(|cards| cards.into_iter().map(|card| card.to_owned()).collect())
        .collect();
